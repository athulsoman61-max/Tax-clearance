...
Although relatively few taxpayers owe federal estate tax, the higher exemption provides additional planning opportunities for high-noth-worth families.
...
`;
    ...
    await db.run(
      "INSERT INTO articles (title, slug, excerpt, content, featured_image, status, reading_time, publish_date, category_id, author_id) VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'), ?, ?)",
      [title, slug, excerpt, content, featured_image, status, reading_time, category_id, author_id]
    );
    res.send("Article added successfully");
  } catch (err) {
    if (err.message.includes('UNIQUE constraint failed')) {
       await db.run("UPDATE articles SET publish_date = datetime('now') WHERE slug = ?", [slug]);
