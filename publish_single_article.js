...
async function publish() {
  ...
  const article = {
    ...
    img: "irs_ai_modernization_2026.png",
...
<p><em>The full ETAAC 2026 Annual Report is available on the <a href="https://www.irs.gov" target="_blank" rel="noopener">IRS website</a>. For personalized guidance on how tax law changes may affect your situation, consult a qualified enrolled agent or tax professional.</em></p>`
  ...
  form.append('excerpt', article.excerpt);
  form.append('category_id', article.category_id);
  form.append('status', article.status);
  form.append('seo_title', article.seo_title);
  form.append('seo_description', article.seo_description);
  form.append('seo_keywords', article.seo_keywords);
  form.append('existing_featured_image', '/img/articles/' + article.img);
  form.append('existing_og_image', '/img/articles/' + article.img);
  ...
  if (saveRes.status === 302) {
    ...
    console.log("⚠️ Status:", saveRes.status, text.substring(0, 200));
  }
}
